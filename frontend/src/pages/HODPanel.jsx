          ...
          <div className="flex gap-1.5 p-1.5 card-main rounded-2xl mb-6 w-full sm:w-fit overflow-x-auto no-scrollbar flex-nowrap">
            {TABS.map(({ id, label, icon: Icon }) => (
              <button key={id} onClick={() => setTab(id)}
                className={`flex items-center gap-2 px-4 sm:px-5 py-2.5 sm:py-2 text-xs font-semibold rounded-xl transition-all cursor-pointer whitespace-nowrap flex-shrink-0 ${tab === id ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'text-slate-500 dark:text-slate-400 hover:text-[var(--text-main)] hover:bg-black/5 dark:hover:bg-white/5'}`}>
                <Icon size={14} /> {label}
              </button>
            ))}
          </div>
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
                      {[
                        ...
                        <div key={label} className="card-main rounded-2xl p-5 flex items-center justify-between sm:block">
                          <div className="text-sm font-semibold text-slate-600 dark:text-slate-400 sm:hidden">{label}</div>
                          <div className="text-right sm:text-left">
                            <div className="text-2xl font-semibold text-[#1D3557]">{val ?? '—'}</div>
                  ...
                  {portal && (
                    <div className="card-main rounded-2xl p-5 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      <div>
                        <h3 className="text-base font-semibold text-[#1D3557]">Department Portal</h3>
                ...
                <form onSubmit={handleCreate} className="flex flex-col gap-5 card-main rounded-2xl p-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-semibold uppercase text-slate-600 dark:text-slate-400 tracking-wider">Section</label>
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-semibold uppercase text-slate-600 dark:text-slate-400 tracking-wider">Faculty & Course</label>
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-semibold uppercase text-slate-600 dark:text-slate-400 tracking-wider">Form Title</label>
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-semibold uppercase text-slate-600 dark:text-slate-400 tracking-wider">Closing Time</label>
                  ...
                  <div className="border-t border-slate-800 pt-5">
                    <div className="flex items-center justify-between mb-3">
                      <label className="text-xs font-semibold uppercase text-slate-600 dark:text-slate-400 tracking-wider">Questions</label>
                      <button type="button" onClick={() => setQuestions([...questions, ''])}
                        className="text-xs font-semibold text-blue-400 hover:text-blue-300 cursor-pointer flex items-center gap-1">
                        <Plus size={13} /> Add Question
                      </button>
                    </div>
                    {questions.map((q, i) => (
                      <div key={i} className="flex gap-2 items-center mb-2">
                        <span className="h-7 w-7 text-xs font-semibold text-blue-300 bg-blue-900/30 border border-blue-800/40 rounded-lg flex items-center justify-center flex-shrink-0">Q{i + 1}</span>
                  ...
                  <button type="submit" disabled={creating}
                    className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white font-semibold py-3.5 px-8 rounded-xl text-sm transition-all cursor-pointer disabled:opacity-75 w-fit">
                    ...
                    <div key={f.id} className="card-main rounded-2xl p-5 flex flex-col md:flex-row md:items-center gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`text-xs font-semibold px-2 py-0.5 rounded-lg border ${statusColor(f.status)}`}>{f.status}</span>
                          <span className="text-xs text-slate-500 dark:text-slate-400 font-mono">{f.responses} responses</span>
                        </div>
                        <div className="text-sm font-semibold text-[#1D3557]">{f.title}</div>
                      ...
                      {!f.expired && (
                        <button onClick={() => toggleForm(f.id, f.is_active)}
                          className={`flex items-center gap-2 px-4 py-2 rounded-xl font-semibold text-sm transition-all cursor-pointer flex-shrink-0 ${f.is_active ? 'bg-accent-600/20 text-accent-400 hover:bg-accent-600/40 border border-accent-800/50' : 'bg-emerald-600/20 text-emerald-400 hover:bg-emerald-600/40 border border-emerald-800/50'}`}>
