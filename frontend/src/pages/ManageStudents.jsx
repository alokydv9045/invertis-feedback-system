          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-10 pb-10 max-w-7xl mx-auto w-full">
            ...
            <div className="card-main relative overflow-hidden group flex flex-col md:flex-row md:items-end justify-between gap-8 p-10">
              ...
              <div className="z-10 flex items-start gap-8">
                ...
                <div>
                  <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-4xl font-semibold text-slate-900 dark:text-white tracking-tight">Student Directory</h1>
                  </div>
                  <p className="text-slate-500 dark:text-slate-400 dark:text-slate-600 dark:text-slate-400 text-sm mt-4 max-w-lg font-semibold leading-relaxed uppercase tracking-widest opacity-80">
                    {user.role === 'admin'
              ...
              <div className="flex items-center gap-8 z-10">
                <div className="hidden lg:flex flex-col items-end">
                  <span className="text-[10px] font-semibold text-slate-600 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em] mb-1">Active Profiles</span>
                  <div className="flex items-baseline gap-1">
                    <span className="text-4xl font-semibold text-slate-900 dark:text-white">{students.length}</span>
                    <span className="text-xs font-semibold text-slate-600 dark:text-slate-400">Synced</span>
                  </div>
                </div>
                <button
                  onClick={() => openModal()}
                  className="flex items-center gap-3 px-10 py-5 bg-primary-600 hover:bg-primary-700 text-white rounded-[2rem] font-semibold text-xs uppercase tracking-[0.15em] transition-all shadow-2xl shadow-primary-500/30 hover:scale-[1.03] active:scale-95 cursor-pointer"
                >
                  <UserPlus size={18} />
            ...
            <div className="flex flex-col lg:flex-row gap-6">
              <div className="flex-1 relative group">
                ...
                <input
                  ...
                  className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] pl-20 pr-10 py-6 text-sm text-slate-800 dark:text-white font-medium focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 transition-all placeholder-slate-400 dark:placeholder-slate-700 shadow-sm"
                  value={search}
              ...
              {user.role === 'admin' && (
                <div className="lg:w-96 relative group">
                  ...
                  <select
                    className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] pl-20 pr-14 py-6 text-sm font-semibold text-slate-700 dark:text-[var(--text-main)] focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 appearance-none cursor-pointer shadow-sm uppercase tracking-widest transition-all"
                    value={selectedDept}
              ...
              <AnimatePresence mode='popLayout'>
                {loading ? (
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="card-main flex flex-col items-center justify-center py-32 text-center">
                    ...
                    <h3 className="text-2xl font-semibold text-slate-900 dark:text-white mb-3">No Records Identified</h3>
                    <p className="text-slate-500 dark:text-slate-400 dark:text-slate-600 dark:text-slate-400 font-semibold uppercase tracking-widest text-[10px] max-w-xs mx-auto">Try broadening your search criteria or scope selection.</p>
                    <button onClick={() => { setSearch(''); setSelectedDept('all'); }} className="mt-10 px-10 py-4 bg-slate-50 dark:bg-slate-800 hover:bg-primary-50 dark:hover:bg-primary-900/30 text-primary-600 dark:text-primary-400 text-[10px] font-semibold rounded-2xl uppercase tracking-[0.2em] transition-all border border-slate-100 dark:border-slate-700">Reset System View</button>
                  </motion.div>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-8 lg:w-[35%] shrink-0">
                      ...
                      <div className="min-w-0">
                        <h3 className="font-semibold text-slate-900 dark:text-white text-2xl tracking-tight leading-tight group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors truncate mb-1">
                          {student.name}
                        </h3>
                        <div className="flex items-center gap-3">
                          <span className="text-[10px] font-semibold text-slate-600 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-[0.2em]">ID: {student.college_id}</span>
                          <div className="h-1 w-1 rounded-full bg-slate-300 dark:bg-slate-700" />
                          <span className="text-[10px] font-semibold text-emerald-500 uppercase tracking-widest">Active</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-10">
                      <div className="flex items-center gap-5">
                        ...
                        <div className="flex flex-col min-w-0">
                          <span className="text-[9px] font-semibold text-slate-600 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Unit</span>
                          <span className="font-semibold text-slate-700 dark:text-[var(--text-main)] text-xs truncate uppercase tracking-tighter">{student.department_name}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-5">
                        ...
                        <div className="flex flex-col min-w-0">
                          <span className="text-[9px] font-semibold text-slate-600 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Correspondence</span>
                          <span className="font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-600 dark:text-slate-400 text-xs truncate">{student.email || 'N/A'}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-5">
                        ...
                        <div className="flex flex-col">
                          <span className="text-[9px] font-semibold text-slate-600 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Load Status</span>
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-semibold text-primary-600 dark:text-primary-400">{student.enrollment_count} Active Modules</span>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            {pagination.totalPages > 1 && (
              <div className="flex items-center justify-between bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-[2rem] shadow-sm">
                <div className="text-[10px] font-semibold text-slate-600 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-widest">
                ...
                <div className="flex items-center gap-4">
                  <button
                    ...
                    className="px-6 py-3 bg-slate-50 dark:bg-slate-800 hover:bg-primary-50 dark:hover:bg-primary-900/30 text-primary-600 dark:text-primary-400 text-[10px] font-semibold rounded-xl uppercase tracking-widest transition-all border border-slate-100 dark:border-slate-700 disabled:opacity-30 disabled:cursor-not-allowed"
                  >
                    Previous
                  </button>
                  <div className="text-xs font-semibold text-slate-700 dark:text-slate-700 dark:text-slate-300">
                    ...
                    <div className="p-12">
                      <div className="flex items-center justify-between mb-12">
                        <div>
                          ...
                          <p className="text-slate-500 dark:text-slate-400 dark:text-slate-600 dark:text-slate-400 text-[10px] mt-3 uppercase tracking-[0.2em] font-semibold opacity-80">Security Protocol Mapping: Enabled</p>
                        </div>
                        <button
                          onClick={() => setShowModal(false)}
                      ...
                      <form onSubmit={handleSubmit} className="space-y-8">
                        ...
                        <div className="flex flex-col gap-3">
                          <label className="text-[10px] font-semibold text-slate-600 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">Faculty Affiliation</label>
                          <select
