...
import {
  BarChart3, Star, UsersRound, BookOpen, Building2,
...
function RatingBadge({ value, max = 7 }) {
  ...
  return <span className={`text-lg font-semibold ${color}`}>{value.toFixed(1)}<span className="text-xs text-slate-500 dark:text-slate-400 font-normal">/{max}</span></span>;
}
...
export default function HODDashboard() {
  ...
  const tabs = [
    ...
    { id: 'faculty', label: 'Faculty Rankings', icon: UsersRound },
            ...
            <div className="flex gap-2 sm:gap-4 border-b border-slate-200 dark:border-slate-800 flex-nowrap overflow-x-auto no-scrollbar pb-1">
              {tabs.map(({ id, label, icon: Icon }) => (
                <button
                  ...
                  className={`flex items-center gap-2 sm:gap-2.5 px-4 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm font-semibold border-b-2 transition -mb-[5px] cursor-pointer uppercase tracking-widest whitespace-nowrap flex-shrink-0 ${
                    activeTab === id
                  ...
                  <div className="flex flex-col gap-8">
                      ...
                      <div key={dept.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl sm:rounded-[2.5rem] p-6 sm:p-10 shadow-sm group relative overflow-hidden">
                        ...
                        <div className="flex items-start justify-between mb-8 sm:mb-10 relative z-10">
                          <div>
                            <div className="flex items-center gap-3 flex-wrap">
                              <h3 className="text-lg sm:text-xl font-semibold text-[var(--text-main)]">{dept.name}</h3>
                              <button
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 relative z-10">
                          {[
                            ...
                            { icon: UsersRound, label: 'Enrolled Students', value: dept.student_count, color: 'text-emerald-500' },
                            ...
                            <div key={label} className="bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50 rounded-2xl p-5 sm:p-6 group-hover:border-primary-500/20 transition-all flex items-center sm:flex-col justify-between sm:justify-normal text-left sm:text-center">
                              <div className="flex items-center gap-3 sm:flex-col sm:gap-2">
                                ...
                                <div className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-widest sm:mt-1">{label}</div>
                              </div>
                              <div className="text-2xl sm:text-3xl font-semibold text-slate-900 dark:text-[var(--text-main)]">{value}</div>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {(data.deptOverview || []).length === 0 && (
                      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] p-20 text-center">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 dark:text-slate-600 dark:text-slate-400 font-semibold uppercase tracking-widest text-xs">No active departmental records.</p>
                      </div>
                    )}
                  </div>
                )}
                ...
                {activeTab === 'faculty' && (
                  <div className="flex flex-col gap-8">
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] p-8 shadow-sm">
                      <h3 className="text-sm font-semibold text-slate-900 dark:text-[var(--text-main)] mb-8 flex items-center gap-3 uppercase tracking-wider">
                      ...
                      {(data.avgRatingPerFaculty || []).length === 0 ? (
                        <div className="p-12 text-center text-slate-600 dark:text-slate-400 font-semibold uppercase text-xs tracking-widest">No evaluation data recorded for faculty.</div>
                      ) : (
                        <div className="h-80">
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={data.avgRatingPerFaculty} layout="vertical" margin={{ left: 10, right: 30 }}>
                              <XAxis type="number" domain={[0, 7]} tick={{ fill: '#475569', fontSize: 11, fontWeight: '600' }} stroke="#cbd5e1" />
                              <YAxis type="category" dataKey="name" tick={{ fill: '#475569', fontSize: 11, fontWeight: '600' }} width={140} stroke="#cbd5e1" />
                              <Tooltip
                                contentStyle={{ 
                      ...
                      {(data.avgRatingPerFaculty || []).map((f, i) => (
                        ...
                        >
                          <div className="flex items-center gap-6">
                            <div className={`h-16 w-16 rounded-2xl flex items-center justify-center text-xl font-semibold text-white shadow-xl transform group-hover:scale-110 transition-transform duration-500 ${
                            ...
                            <div className="flex-1 min-w-0">
                              <h3 className="font-semibold text-slate-900 dark:text-white text-lg truncate group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">{f.name}</h3>
                              <div className="text-[10px] text-slate-600 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 font-semibold truncate uppercase tracking-[0.1em] mt-1.5">{f.total_responses} Evaluation Samples</div>
                            </div>
                          </div>
                          <div className="flex items-center justify-between mt-8 pt-6 border-t border-slate-100 dark:border-slate-800">
                             <div className="flex items-center gap-1.5">
                                <Star size={18} className="text-amber-400 fill-amber-400" />
                                <span className={`text-2xl font-semibold ${f.avg_rating >= 5 ? 'text-emerald-500' : f.avg_rating >= 3.5 ? 'text-amber-500' : 'text-accent-500'}`}>
                                  {f.avg_rating.toFixed(1)}
                                </span>
                                <span className="text-xs text-slate-600 dark:text-slate-400 font-semibold uppercase">Rating</span>
                             </div>
                          </div>
                        </motion.div>
                      ))}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {(data.submissionRates || []).map(c => (
                      <div key={c.course_id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] p-8 shadow-sm group">
                        <div className="flex items-start justify-between mb-6">
                          <div className="min-w-0">
                            <div className="flex items-center gap-3 mb-2">
                              <span className="text-[10px] font-semibold bg-primary-50 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400 px-2.5 py-1 rounded-lg border border-primary-100 dark:border-primary-800/30 uppercase tracking-tighter">{c.course_code}</span>
                              <h4 className="text-base font-semibold text-slate-900 dark:text-[var(--text-main)] truncate group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">{c.course_name}</h4>
                            </div>
                            <p className="text-[10px] text-slate-600 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-widest">{c.department_name}</p>
                          </div>
                          <div className="text-right shrink-0 ml-4">
                            <span className={`text-3xl font-semibold ${c.rate >= 70 ? 'text-emerald-500' : c.rate >= 40 ? 'text-amber-500' : 'text-accent-500'}`}>{c.rate}%</span>
                            <div className="text-[9px] font-semibold text-slate-600 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-1">Submission Rate</div>
                          </div>
                        </div>
                        ...
                        <div className="flex gap-6 mt-6 pt-4 border-t border-slate-50 dark:border-slate-800/50 text-[10px] font-semibold uppercase tracking-[0.1em]">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {filteredComments.length === 0 ? (
                          <div className="col-span-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] p-20 text-center shadow-sm">
                             ...
                             <p className="text-slate-500 dark:text-slate-400 dark:text-slate-600 dark:text-slate-400 font-semibold uppercase tracking-widest text-xs">No feedback narratives received yet.</p>
                          </div>
                        ) : (
                            ...
                            >
                              <div className="flex justify-between items-center mb-6">
                                 <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-semibold text-slate-600 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-widest">{c.faculty_name}</span>
                                 </div>
                              ...
                              <div className="flex items-center justify-between mt-8 pt-6 border-t border-slate-50 dark:border-slate-800/50 text-[10px] font-semibold text-slate-600 dark:text-slate-400 dark:text-slate-600 uppercase tracking-widest">
