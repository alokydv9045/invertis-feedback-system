        ...
        <div className="flex flex-col md:flex-row flex-1">
          ...
          <main className="flex-1 p-6 flex items-center justify-center">
            ...
            >
              ...
              <div>
                <h2 className="text-3xl font-semibold text-slate-900 dark:text-[var(--text-main)] tracking-tight">Submission Successful</h2>
              ...
              <button
                ...
                className="w-full bg-primary-600 hover:bg-primary-700 text-white py-4 rounded-2xl font-semibold text-xs uppercase tracking-widest transition-all shadow-xl shadow-primary-500/20 cursor-pointer flex items-center justify-center gap-3"
              >
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-10 max-w-4xl mx-auto w-full">
            ...
            <div className="flex items-start justify-between gap-6 flex-wrap">
              <div>
                <button
                  onClick={() => navigate(-1)}
                  className="hidden md:flex items-center gap-1.5 text-[10px] font-semibold text-slate-600 dark:text-slate-400 hover:text-primary-600 dark:hover:text-primary-400 transition mb-4 cursor-pointer uppercase tracking-widest"
                >
                  <ArrowLeft size={14} /> Back to Course
                </button>
                <h1 className="text-3xl md:text-4xl font-semibold tracking-tight">TLFQ Sheet</h1>
              </div>
              <div className="flex gap-3">
                <span className="flex items-center gap-2 text-[10px] font-semibold bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/40 text-emerald-600 dark:text-emerald-400 px-4 py-2 rounded-xl uppercase tracking-widest shadow-sm">
                  <Lock size={12} /> Anonymous
                </span>
                <span className="flex items-center gap-2 text-[10px] font-semibold bg-primary-50 dark:bg-primary-900/20 border border-primary-100 dark:border-primary-800/40 text-primary-600 dark:text-primary-400 px-4 py-2 rounded-xl uppercase tracking-widest shadow-sm">
            ...
            {loading ? (
              <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] p-24 flex flex-col items-center gap-4 shadow-sm">
                ...
                <span className="text-xs font-semibold uppercase tracking-widest text-slate-600 dark:text-slate-400 animate-pulse">Initializing Questionnaire...</span>
              </div>
            ) : !evaluation ? (
              <div className="bg-accent-50 dark:bg-accent-950/20 border border-accent-100 dark:border-accent-900/40 text-accent-600 dark:text-accent-400 p-8 rounded-[2rem] text-sm font-semibold text-center shadow-sm">Failed to load questionnaire.</div>
            ) : (
              <form onSubmit={handleSubmit} className="flex flex-col gap-10">
                ...
                <div className="bg-white dark:bg-slate-900 border border-primary-100 dark:border-primary-900/40 rounded-[2.5rem] p-8 flex justify-between items-center shadow-sm">
                  <div className="min-w-0">
                    <h2 className="text-xl font-semibold text-slate-900 dark:text-[var(--text-main)] truncate">{evaluation?.title}</h2>
                    <div className="flex items-center gap-4 mt-2">
                       <div className="flex items-center gap-1.5">
                          <Users size={12} className="text-slate-600 dark:text-slate-400" />
                          <span className="text-[11px] text-slate-500 dark:text-slate-400 font-semibold">{evaluation?.faculty_name}</span>
                       </div>
                       <div className="h-1 w-1 rounded-full bg-slate-300 dark:bg-slate-700" />
                       <div className="flex items-center gap-1.5">
                          <BookOpen size={12} className="text-slate-600 dark:text-slate-400" />
                          <span className="text-[11px] text-slate-500 dark:text-slate-400 font-semibold">{evaluation?.course_name}</span>
                       </div>
                    </div>
                  </div>
                  <button
                    type="button" onClick={clearDraft}
                    className="text-[10px] font-semibold text-slate-600 dark:text-slate-400 hover:text-accent-500 flex items-center gap-2 px-4 py-2 hover:bg-accent-50 dark:hover:bg-accent-950/30 rounded-xl transition cursor-pointer uppercase tracking-widest shrink-0"
                  >
                ...
                <div className="flex flex-col gap-3">
                   <div className="flex items-center justify-between text-[10px] font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-widest">
                    ...
                    >
                      <div className="flex justify-between items-start gap-6">
                        <div className="flex gap-5">
                          <span className="shrink-0 flex items-center justify-center h-10 w-10 bg-primary-50 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400 font-semibold rounded-2xl text-sm border border-primary-100 dark:border-primary-800/30 shadow-sm">
                            {idx + 1}
                          </span>
                          <p className="text-base md:text-lg font-semibold text-slate-800 dark:text-[var(--text-main)] leading-tight pt-1.5">{q.question_text}</p>
                        </div>
                        <button
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] p-8 flex flex-col gap-6 shadow-sm">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <h3 className="text-lg font-semibold text-slate-900 dark:text-[var(--text-main)]">Narrative Feedback</h3>
                      <p className="text-xs text-slate-600 dark:text-slate-400 dark:text-slate-500 dark:text-slate-400 mt-1 font-semibold uppercase tracking-widest">Optional subjective input</p>
                    </div>
                    {voiceSupported && (
                      <button
                        type="button" onClick={startVoiceInput}
                        className={`flex items-center gap-2 px-5 py-2.5 text-[10px] font-semibold rounded-xl transition border uppercase tracking-widest cursor-pointer shadow-sm ${
                          listening
                ...
                {/* Submit */}
                 <div className="flex gap-6 pt-4">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-3 bg-primary-600 hover:bg-primary-700 text-white font-semibold py-5 rounded-[2rem] text-sm transition-all shadow-2xl shadow-primary-500/30 cursor-pointer disabled:opacity-70 uppercase tracking-[0.2em]"
                  >
                    {submitting
                  ...
                  <button
                    ...
                    className="bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 dark:text-slate-600 dark:text-slate-400 font-semibold py-5 px-12 rounded-[2rem] text-sm border border-slate-200 dark:border-slate-800 transition cursor-pointer uppercase tracking-widest shadow-sm"
                  >
                    Discard
                  </button>
                </div>
