...
import { Plus, Trash2, Check, X, UsersRound, BookOpen, GraduationCap, Link2, Building2, ChevronDown, Key, Upload, FileText, Info, Layers } from 'lucide-react';

const TABS = [
  { id: 'sections', label: 'Sections', icon: Layers },
  { id: 'courses', label: 'Courses', icon: BookOpen },
  { id: 'faculty', label: 'Faculty', icon: UsersRound },
...
function Label({ children }) {
  return <label className="text-[10px] font-semibold uppercase text-slate-500 dark:text-slate-400 tracking-widest">{children}</label>;
}
...
function Btn({ children, variant = 'primary', ...props }) {
  const cls = variant === 'primary'
    ? 'btn-primary'
    : variant === 'danger'
      ? 'bg-accent-600/20 hover:bg-accent-600 hover:text-white text-accent-400 border border-accent-500/30 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all'
      : 'bg-slate-500/10 hover:bg-slate-500/20 text-slate-500 dark:text-slate-400 hover:text-[var(--text-main)] border border-[var(--border-base)] px-5 py-2.5 rounded-xl text-sm font-semibold transition-all';
...
function DepartmentsTab({ departments, onRefresh }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <Card>
        <h3 className="text-sm font-semibold text-[var(--text-main)] mb-3">Add Department</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {departments.map(d => (
          <Card key={d.id} className="flex items-center justify-between">
            <div>
              <div className="text-sm font-semibold text-[var(--text-main)]">{d.name}</div>
              <div className="flex items-center gap-2 mt-0.5">
                <div className="text-xs text-slate-500 dark:text-slate-400 font-mono">{d.code}</div>
                <div className="text-[10px] bg-slate-500/10 text-slate-500 dark:text-slate-400 px-1.5 py-0.5 rounded font-semibold">{d.max_semester || 8} Sems</div>
              </div>
            </div>
...
function SectionsTab({ departments, sections, onRefresh }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <Card>
        <h3 className="text-sm font-semibold text-[var(--text-main)] mb-3">Create Section</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {sections.map(s => (
          <Card key={s.id} className="flex items-center justify-between">
            <div>
              <div className="text-sm font-semibold text-[var(--text-main)]">{s.name}</div>
...
function CoursesTab({ departments, courses, onRefresh }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <Card>
        <h3 className="text-sm font-semibold text-[var(--text-main)] mb-3">Add Course</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {courses.map(c => (
          <Card key={c.id} className="flex items-center justify-between">
            <div>
              <div className="text-sm font-semibold text-[var(--text-main)]">{c.name}</div>
...
function FacultyTab({ departments, faculty, onRefresh }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <Card>
        <h3 className="text-sm font-semibold text-[var(--text-main)] mb-3">Add Faculty Member</h3>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {faculty.map(f => (
          <Card key={f.id} className="flex items-center justify-between">
            <div>
              <div className="text-sm font-semibold text-[var(--text-main)]">{f.name}</div>
              <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{f.department_name}</div>
              <span className={`inline-flex mt-1.5 text-[10px] font-semibold px-2 py-0.5 rounded-md border ${f.teacher_type === 'trainer'
...
function AssignmentsTab({ departments, sections, faculty, courses, onRefresh }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <Card>
        <h3 className="text-sm font-semibold text-[var(--text-main)] mb-3">Assign Faculty to Section</h3>
...
function StudentsTab({ departments, sections, students, onRefresh }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <Card>
        <h3 className="text-sm font-semibold text-[var(--text-main)] mb-3">Pre-Create Student Account</h3>
      ...
      <Card className="border border-primary-500/20 bg-primary-950/10">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-sm font-semibold text-[var(--text-main)]">Bulk Import via CSV</h3>
        </div>
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-3">
              <label className="flex items-center gap-2 bg-primary-600 hover:bg-primary-500 text-white px-5 py-2.5 rounded-xl text-xs font-semibold uppercase tracking-widest cursor-pointer transition-all shadow-lg shadow-primary-500/20 active:scale-95">
                <FileText size={14} />
          ...
          <div className="w-full md:w-64 bg-slate-950/50 rounded-2xl p-4 border border-white/5">
            <h4 className="text-[9px] font-semibold uppercase text-slate-500 dark:text-slate-400 tracking-[0.2em] mb-3 flex items-center gap-2">
        ...
        {filteredStudents.map(s => (
          <Card key={s.id} className="flex flex-col gap-3">
            <div className="flex items-start justify-between">
              ...
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-lg ${s.status === 'active' ? 'bg-emerald-900/40 text-emerald-400' : 'bg-amber-900/40 text-amber-400'}`}>
                {s.status}
              </span>
            </div>
            {resetId === s.id ? (
              ...
              <button onClick={() => setResetId(s.id)} className="flex items-center gap-2 text-xs font-semibold text-slate-600 dark:text-slate-400 hover:text-primary-400 transition-colors cursor-pointer">
      ...
      <div className="admin-layout">
        ...
        <main className="admin-content">
          <header className="mb-8">
            <h1 className="text-2xl font-semibold gradient-text mb-1">Coordinator Hub</h1>
