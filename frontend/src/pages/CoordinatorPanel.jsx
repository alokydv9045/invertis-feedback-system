  ...
  return (<div className="space-y-4">
    <Card><CardBody><h3 className="text-sm font-bold text-gray-900 mb-3">Add Department</h3>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div><Label>Name</Label><input value={name} onChange={e => setName(e.target.value)} placeholder="B.Tech Computer Science" className={inputCls} /></div>
        <div><Label>Code</Label><input value={code} onChange={e => setCode(e.target.value.toUpperCase())} placeholder="BCS" className={inputCls} /></div>
        <div className="flex items-end"><Button icon={Plus} onClick={create}>Create</Button></div>
      </div>
    </CardBody></Card>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      {departments.map(d => (<Card key={d.id}><CardBody className="py-3 flex items-center justify-between">
        <div><p className="text-sm font-bold text-gray-900">{d.name}</p><p className="text-xs text-gray-400 font-mono">{d.code}</p></div>
        <button onClick={() => del(d.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg cursor-pointer"><Trash2 size={16} /></button>
      </CardBody></Card>))}
    </div>
  </div>);
}
  ...
  return (<div className="space-y-4">
    <Card><CardBody><h3 className="text-sm font-bold text-gray-900 mb-3">Create Section</h3>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="col-span-2 md:col-span-1"><Label>Department</Label><select value={deptId} onChange={e => setDeptId(e.target.value)} className={inputCls}><option value="">Select…</option>{departments.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select></div>
        <div><Label>Semester</Label><select value={sem} onChange={e => setSem(e.target.value)} className={inputCls}>{[1,2,3,4,5,6,7,8].map(s => <option key={s} value={s}>Sem {s}</option>)}</select></div>
        <div><Label>Section</Label><select value={label} onChange={e => setLabel(e.target.value)} className={inputCls}>{Array.from({length:26},(_,i)=>String.fromCharCode(65+i)).map(l => <option key={l} value={l}>{l}</option>)}</select></div>
        <div className="flex items-end"><Button icon={Plus} onClick={create}>Create</Button></div>
      </div>
    </CardBody></Card>
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
      {sections.map(s => (<Card key={s.id}><CardBody className="py-3 flex items-center justify-between">
        ...
        <button onClick={() => del(s.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg cursor-pointer"><Trash2 size={16} /></button>
      </CardBody></Card>))}
    </div>
  </div>);
}
...
function CoursesTab({ departments, courses, onRefresh, setMsg }) {
  ...
  return (<div className="space-y-4">
    <Card><CardBody><h3 className="text-sm font-bold text-gray-900 mb-3">Add Course</h3>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="col-span-2 md:col-span-1"><Label>Course Name</Label><input value={name} onChange={e => setName(e.target.value)} placeholder="Data Structures" className={inputCls} /></div>
        <div><Label>Code</Label><input value={code} onChange={e => setCode(e.target.value.toUpperCase())} placeholder="BCS201" className={inputCls} /></div>
        <div><Label>Department</Label><select value={deptId} onChange={e => setDeptId(e.target.value)} className={inputCls}><option value="">Select…</option>{departments.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select></div>
        <div className="flex items-end"><Button icon={Plus} onClick={create}>Add</Button></div>
      </div>
    </CardBody></Card>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      {courses.map(c => (<Card key={c.id}><CardBody className="py-3 flex items-center justify-between">
        ...
        <button onClick={() => del(c.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg cursor-pointer"><Trash2 size={16} /></button>
      </CardBody></Card>))}
    </div>
  </div>);
}
  ...
  return (<div className="space-y-4">
    <Card><CardBody><h3 className="text-sm font-bold text-gray-900 mb-3">Add Faculty Member</h3>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        <div className="md:col-span-2"><Label>Faculty Name</Label><input value={name} onChange={e => setName(e.target.value)} placeholder="Dr. Alan Turing" className={inputCls} /></div>
        <div><Label>Department</Label><select value={deptId} onChange={e => setDeptId(e.target.value)} className={inputCls}><option value="">Select…</option>{departments.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select></div>
        <div><Label>Type</Label><select value={teacherType} onChange={e => setTeacherType(e.target.value)} className={inputCls}><option value="college_faculty">College Faculty</option><option value="trainer">Trainer</option></select></div>
      </div>
      <Button icon={Plus} onClick={create} className="mt-3">Add Faculty</Button>
    </CardBody></Card>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      {faculty.map(f => (<Card key={f.id}><CardBody className="py-3 flex items-center justify-between">
        ...
        <button onClick={() => del(f.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg cursor-pointer"><Trash2 size={16} /></button>
      </CardBody></Card>))}
    </div>
  </div>);
}
  ...
  return (<div className="space-y-4">
    <Card><CardBody><h3 className="text-sm font-bold text-gray-900 mb-3">Assign Faculty to Section</h3>
      <div className="space-y-3">
        <div><Label>Filter by Department</Label><select value={deptFilter} onChange={e => setDeptFilter(e.target.value)} className={inputCls}><option value="">All</option>{departments.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select></div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div><Label>Section</Label><select value={sectionId} onChange={e => setSectionId(e.target.value)} className={inputCls}><option value="">Select…</option>{fSections.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
          <div><Label>Faculty</Label><select value={facultyId} onChange={e => setFacultyId(e.target.value)} className={inputCls}><option value="">Select…</option>{fFaculty.map(f => <option key={f.id} value={f.id}>{f.name}</option>)}</select></div>
          <div><Label>Course</Label><select value={courseId} onChange={e => setCourseId(e.target.value)} className={inputCls}><option value="">Select…</option>{fCourses.map(c => <option key={c.id} value={c.id}>[{c.code}] {c.name}</option>)}</select></div>
        </div>
  ...
  return (<div className="space-y-4">
    <Card><CardBody><h3 className="text-sm font-bold text-gray-900 mb-3">Pre-Create Student Account</h3>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <div><Label>Full Name</Label><input value={name} onChange={e => setName(e.target.value)} placeholder="Rahul Sharma" className={inputCls} /></div>
        <div><Label>Student ID</Label><input value={stdId} onChange={e => setStdId(e.target.value.toUpperCase())} placeholder="BCS2025_55" className={`${inputCls} font-mono`} /></div>
        <div><Label>Department</Label><select value={deptId} onChange={e => { setDeptId(e.target.value); setSectionId(''); }} className={inputCls}><option value="">Select…</option>{departments.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select></div>
        <div><Label>Section</Label><select value={sectionId} onChange={e => setSectionId(e.target.value)} className={inputCls}><option value="">Select…</option>{fSections.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
        <div><Label>Semester</Label><select value={sem} onChange={e => setSem(e.target.value)} className={inputCls}>{[1,2,3,4,5,6,7,8].map(s => <option key={s} value={s}>Sem {s}</option>)}</select></div>
        <div><Label>Batch</Label><input value={batch} onChange={e => setBatch(e.target.value)} placeholder="2025" className={inputCls} /></div>
      </div>
    ...
    <div className="flex items-center gap-3 mb-1">
      ...
      <select value={filterDept} onChange={e => setFilterDept(e.target.value)} className={`${inputCls} w-48`}><option value="">All</option>{departments.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select>
    </div>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      {fStudents.map(s => (<Card key={s.id}><CardBody className="py-3 space-y-2">
        ...
        {resetId === s.id ? (
          <div className="flex gap-2"><input value={newPwd} onChange={e => setNewPwd(e.target.value)} placeholder="New password (min 8)" type="password" className={inputCls} />
            <Button size="sm" icon={Check} onClick={() => resetPwd(s.id)} /><Button size="sm" variant="secondary" icon={X} onClick={() => setResetId('')} /></div>
        ) : (
...
export default function CoordinatorPanel() {
  ...
  return (
    <div className="animate-fade-in max-w-6xl">
      ...
      {msg && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`mb-4 p-3 border text-sm font-semibold rounded-lg flex items-center justify-between ${msg.type === 'success' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-red-50 text-red-600 border-red-200'}`}>
      ...
      <div className="flex bg-gray-100 rounded-lg p-1 mb-6 flex-wrap">
        {TABS.map(({ id, label, icon: Icon }) => (
          <button key={id} onClick={() => setTab(id)} className={`flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-md transition-all cursor-pointer ${tab === id ? 'bg-white text-invertis-blue shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
            <Icon size={14} /> {label}
          </button>
        ))}
      </div>
