    ...
    <div className="min-h-screen bg-[var(--bg-main)] text-[var(--text-main)] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        ...
        <div className="flex flex-col items-center mb-8 text-center">
          ...
          <h1 className="text-2xl font-semibold text-slate-900 dark:text-white">Student Portal</h1>
          ...
          {[
            ...
            <div key={`step-wrap-${n}`} className="flex items-center gap-2 flex-1">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                <div className={`h-7 w-7 rounded-lg flex items-center justify-center text-xs font-semibold transition-all ${
                ...
                <span className={`text-[11px] font-semibold ${step === n ? 'text-[var(--text-main)]' : 'text-slate-500'}`}>{label}</span>
              </div>
            </div>
          ))}
        </div>
          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              <motion.div key="step1" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }}>
                <h2 className="text-base font-semibold text-slate-900 dark:text-white mb-1">Enter your Student ID</h2>
            ...
            {step === 2 && (
              <motion.div key="step2" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }}>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h2 className="text-base font-semibold text-slate-900 dark:text-white">Hello, {studentName}!</h2>
                </div>
