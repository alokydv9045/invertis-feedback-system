...
import {
  ...
  X, Check, Eye, EyeOff, Megaphone, ChevronRight, Menu, Briefcase, ClipboardList, ShieldCheck
} from 'lucide-react';
...
const ROLE_CONFIG = {
  supreme:     { label: 'Supreme Authority', icon: Shield,        color: '#F59E0B' },
  super_admin: { label: 'Super Admin',        icon: ShieldCheck,   color: '#60A5FA' },
  coordinator: { label: 'Coordinator',        icon: ClipboardList, color: '#34D399' },
  hod:         { label: 'Head of Dept',       icon: Briefcase,     color: '#A78BFA' },
      ...
      >
        ...
        <div className="p-5 flex flex-col gap-3.5">
          {[
            ...
            <div key={f.label}>
              <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-widest mb-1">{f.label}</label>
...
export default function Navbar() {
  ...
  const { toggle } = useSidebarItems();
  const role = ROLE_CONFIG[user?.role] || ROLE_CONFIG.student;
  const RoleIcon = role.icon;

  return (
    <>
      {/* ── Row 2: Pure White Logo Header (ERP style) ──────────────────────────── */}
      <div className="w-full bg-white px-4 py-2.5 flex items-center justify-between shadow-sm shrink-0 border-b border-slate-200 relative z-30">
        {/* Left Section: Logo & Mobile Hamburger */}
        <div className="flex items-center gap-2.5">
          {user && (
            <button
              onClick={toggle}
              className="md:hidden p-2 rounded-xl text-slate-500 hover:bg-slate-50 transition-colors focus:outline-none cursor-pointer"
        ...
        {user && (
          <div className="flex items-center gap-2">
            ...
            <div className="hidden md:flex items-center gap-2.5">
              <div className="h-8 w-8 rounded-xl bg-[#1D3557]/5 flex items-center justify-center text-[#1D3557] font-semibold text-sm shrink-0 border border-[#1D3557]/10">
                {user.name?.charAt(0) || 'U'}
              </div>
              <div className="text-right">
                <div className="text-xs font-semibold text-slate-800 leading-none">{user.name}</div>
                <div className="flex items-center gap-1 justify-end mt-1">
                  <RoleIcon size={9} style={{ color: '#FF5A36' }} />
                  <span className="text-[10px] text-slate-500 font-medium">{role.label}</span>
                </div>
              </div>
            </div>
            ...
            {CAN_CHANGE_PASSWORD.includes(user?.role) && (
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 rounded-xl text-[11px] font-semibold text-slate-700 hover:bg-slate-50 border border-slate-200 transition-all cursor-pointer"
              >
                <KeyRound size={12} />
            ...
            <button
              onClick={logout}
              className="flex items-center gap-1 px-3 py-1.5 rounded-xl text-[11px] font-semibold text-white bg-[#FF5A36] hover:bg-[#E24825] shadow-sm shadow-[#FF5A36]/15 hover:shadow-md transition-all cursor-pointer"
            >
              <LogOut size={12} />
      ...
      {/* ── Orange divider ───────────────────────────────────────── */}
      <div className="w-full h-[3px] bg-[#FF5A36] shrink-0 relative z-30" />
