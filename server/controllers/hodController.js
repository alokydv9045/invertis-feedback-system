import { Section, SectionFaculty, Faculty, Course, Tlfq, Question, Response, User, Department } from '../db.js';

// ── GET /api/hod/sections — sections in HOD's dept ────────────────────────
export const getHodSections = async (req, res) => {
  try {
    const { department_id } = req.user;
    const data = await Section.findMany({ where: { department_id } });
...
export const getSectionFaculty = async (req, res) => {
  try {
    ...
    const list = await SectionFaculty.findMany({
      where: { section_id },
      include: { faculty: { select: { name: true } }, course: { select: { name: true, code: true } } }
    });
...
export const createTlfq = async (req, res) => {
  try {
    ...
    const section = await Section.findUnique({ where: { id: section_id }, select: { department_id: true } });
    ...
    if (questions.length > 0) {
      for (const q of questions) {
        await Question.create({ data: { tlfq_id: tlfq.id, question_text: q } });
      }
    }
...
export const toggleForm = async (req, res) => {
  try {
    ...
    const form = await Tlfq.findUnique({ where: { id: req.params.id }, select: { created_by: true } });
    if (!form) return res.status(404).json({ message: 'Form not found.' });
    if (form.created_by !== req.user.id) {
      return res.status(403).json({ message: 'You can only manage your own forms.' });
    }
    await Tlfq.update({ where: { id: req.params.id }, data: { is_active: !!is_active } });
...
export const updateDeadline = async (req, res) => {
  try {
    ...
    await Tlfq.update({ where: { id: req.params.id }, data: { closing_time: new Date(closing_time) } });
...
export const getHodStats = async (req, res) => {
  try {
    ...
    const [sections, faculty, courses, students, myForms, openForms] = await Promise.all([
      ...
      Tlfq.count({ where: { created_by: req.user.id, is_active: true, closing_time: { gt: now } } }),
    ]);
...
export const getPortalStatus = async (req, res) => {
  try {
    const dept = await Department.findUnique({
      where: { id: req.user.department_id },
      select: { portal_open: true, name: true }
    });
    if (!dept) return res.status(404).json({ message: 'Department not found' });
