...
export default function SuperAdminPanel() {
  ...
  return (
    <div className="animate-fade-in max-w-5xl">
      ...
      <div className="flex bg-gray-100 rounded-lg p-1 mb-6 w-fit flex-wrap">
            ...
            <div className="space-y-4">
              <Card><CardBody>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div><label className="block text-sm font-semibold text-gray-700 mb-1">Name</label><input value={deptName} onChange={e => setDeptName(e.target.value)} placeholder="B.Tech Computer Science" className={inputClass} /></div>
                  <div><label className="block text-sm font-semibold text-gray-700 mb-1">Code</label><input value={deptCode} onChange={e => setDeptCode(e.target.value.toUpperCase())} placeholder="BCS" className={inputClass} /></div>
                  <div className="flex items-end"><Button icon={Plus} onClick={createDept}>Create</Button></div>
                </div>
              </CardBody></Card>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {departments.map(d => (
                  <Card key={d.id}><CardBody className="py-3 flex items-center justify-between">
                    <div><p className="text-sm font-bold text-gray-900">{d.name}</p><p className="text-xs text-gray-400 font-mono">{d.code}</p></div>
                    <button onClick={() => deleteDept(d.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg cursor-pointer"><Trash2 size={16} /></button>
                  </CardBody></Card>
                ))}
              </div>
            </div>
          ...
          {tab === 'hods' && (
            <div className="space-y-4">
              <Card><CardBody>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div><label className="block text-sm font-semibold text-gray-700 mb-1">Full Name</label><input value={hodName} onChange={e => setHodName(e.target.value)} placeholder="Dr. Rajesh Kumar" className={inputClass} /></div>
                  <div><label className="block text-sm font-semibold text-gray-700 mb-1">Email</label><input type="email" value={hodEmail} onChange={e => setHodEmail(e.target.value)} placeholder="hod@invertis.edu.in" className={inputClass} /></div>
                  <div><label className="block text-sm font-semibold text-gray-700 mb-1">Password</label>
                    <div className="relative"><input type={showHodPass ? 'text' : 'password'} value={hodPass} onChange={e => setHodPass(e.target.value)} placeholder="Min. 8 characters" className={inputClass} />
                    <button type="button" onClick={() => setShowHodPass(!showHodPass)} className="absolute right-3 top-3 text-gray-400 hover:text-gray-600 cursor-pointer">{showHodPass ? <EyeOff size={16} /> : <Eye size={16} />}</button></div>
                  </div>
                  <div><label className="block text-sm font-semibold text-gray-700 mb-1">Department</label>
                    <select value={hodDept} onChange={e => setHodDept(e.target.value)} className={inputClass}><option value="">Select…</option>{departments.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select>
                  </div>
                </div>
                <Button icon={Check} onClick={createHod} className="mt-3">Create HOD</Button>
              </CardBody></Card>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {hods.map(h => { const dept = departments.find(d => d.id === h.department_id); return (
                  <Card key={h.id}><CardBody className="py-3 flex items-center justify-between">
                    ...
                    <button onClick={() => deleteUser(h.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg cursor-pointer"><Trash2 size={16} /></button>
                  </CardBody></Card>
                ); })}
              </div>
            </div>
          ...
          {tab === 'coordinators' && (
            <div className="space-y-4">
              <Card><CardBody>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div><label className="block text-sm font-semibold text-gray-700 mb-1">Full Name</label><input value={coordName} onChange={e => setCoordName(e.target.value)} placeholder="Academic Coordinator" className={inputClass} /></div>
                  <div><label className="block text-sm font-semibold text-gray-700 mb-1">Email</label><input type="email" value={coordEmail} onChange={e => setCoordEmail(e.target.value)} placeholder="coordinator@invertis.edu.in" className={inputClass} /></div>
                  <div><label className="block text-sm font-semibold text-gray-700 mb-1">Password</label>
                    <div className="relative"><input type={showCoordPass ? 'text' : 'password'} value={coordPass} onChange={e => setCoordPass(e.target.value)} placeholder="Min. 8 characters" className={inputClass} />
                    <button type="button" onClick={() => setShowCoordPass(!showCoordPass)} className="absolute right-3 top-3 text-gray-400 hover:text-gray-600 cursor-pointer">{showCoordPass ? <EyeOff size={16} /> : <Eye size={16} />}</button></div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {coords.map(c => (
                  <Card key={c.id}><CardBody className="py-3 flex items-center justify-between">
                    ...
                    <button onClick={() => deleteUser(c.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg cursor-pointer"><Trash2 size={16} /></button>
                  </CardBody></Card>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-4">
              <Card className="border-emerald-200 bg-emerald-50/30"><CardBody>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search by Anonymous ID or real name..."
                    className={`${inputClass} pl-11`} />
                  {searchQuery && <button onClick={() => setSearchQuery('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 cursor-pointer"><X size={14} /></button>}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {filteredStudents.map(s => {
                  ...
                  return (
                    <Card key={s.id} className="hover:border-emerald-200 transition-colors">
